...
if len(sys.argv) < 8:
    ...
    print "python lucene_model_trec_script_generator.py path_to_lucene query_file collection model param_setting title_boost content_boost"
    sys.exit(1)
...
print '%s/params/%s/trec_files/%s/%s-%.2f-%s-%s.params' % (path_to_lucene,collection,fmodel,fmodel,param_setting,title_boost,content_boost)

filename = '%s/params/%s/trec_files/%s/%s-%.2f-%s-%s.params' % (path_to_lucene,collection,fmodel,fmodel,param_setting,title_boost,content_boost)
...
<model>%s</model> \n\
<%s>%.2f</%s> \n\
<resultFile>%s/data/%s/trec_files/%s/%s-%.2f-%s-%s.res</resultFile> \n\
<tokenFilterFile>%s/params/news_token_filters.xml</tokenFilterFile> \n\
<fieldsFile>%s/params/%s/trec_files/%s/%s-%.2f-%s-%s.fieldparams</fieldsFile> \n\
...
filename = '%s/params/%s/trec_files/%s/%s-%.2f-%s-%s.fieldparams' % (path_to_lucene,collection,fmodel,fmodel,param_setting,title_boost,content_boost)
file = open(filename, 'w')
file.write('<?xml version="1.0" encoding="UTF-8" standalone="yes"?> \n\
<fields> \n\
<field> \n\
